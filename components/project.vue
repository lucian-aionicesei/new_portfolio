    ...
    <article v-if="showProject" class="fixed top-0 left-0 z-50 h-screen w-screen justify-end pt-12 dark:text-primary">
          ...
          <div class="fixed z-20 bottom-0 right-0 pr-6 pb-6">
            <div>
              ...
              <svg @click="bringNext" class="h-12 w-auto hover:scale-125 ease-out transition-all cursor-pointer" viewBox="0 0 55 55" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <div class="fixed z-20 bottom-0 left-0 pl-6 pb-6">
            <div>
              ...
              <svg @click="bringPrevious" class="h-12 w-auto hover:scale-125 ease-out transition-all cursor-pointer" viewBox="0 0 55 55" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const bringNext = () => {
  // console.log("bring next project", coreStore.projectIndex + 1);
  // console.log("array length", coreStore.projects.length);
  if (coreStore.projectIndex < coreStore.projects.length - 1) {
    coreStore.projectIndex = coreStore.projectIndex + 1;
  } else {
    coreStore.projectIndex = 0;
  }
};

const bringPrevious = () => {
  // console.log("bring previous project");
  if (coreStore.projectIndex === 0) {
    coreStore.projectIndex = coreStore.projects.length - 1;
  } else {
    coreStore.projectIndex = coreStore.projectIndex - 1;
  }
};
